...
def extract_fields(body: dict | None) -> Dict[str, Any]:
    ...
    for t in body.get("totals", []):
        if isinstance(t, dict) and "amount" in t:
            mapping = {"subtotal": "subtotal_amount", "tax": "tax_amount",
                       "shipping": "shipping_amount", "discount": "discount_amount",
                       "total": "total_amount"}
...
async def create_checkout(request: Request):
    ...
    session = {
        ...
        "payment": {
            "handlers": [
                {"id": "mock_payment_handler", "type": "card", "brand": "Visa"},
            ],
        },
        "messages": [],
    }
...
async def update_checkout(session_id: str, request: Request):
    ...
    if "fulfillment" in body:
        ...
        # Add shipping cost
        shipping = 599
        session["totals"].append({"type": "shipping", "amount": shipping})
        # Recalculate total
...
def print_analytics_report(conn: sqlite3.Connection, session_id: str):
    ...
    cursor = conn.execute("""
        SELECT
            ...
            MAX(shipping_amount) as shipping,
    ...
    for row in cursor.fetchall():
        currency, subtotal, tax, shipping, discount, total = row
        print(f"   Currency:  {currency}")
        print(f"   Subtotal:  ${(subtotal or 0) / 100:.2f}")
        print(f"   Tax:       ${(tax or 0) / 100:.2f}")
        print(f"   Shipping:  ${(shipping or 0) / 100:.2f}")
